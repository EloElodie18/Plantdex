...
export class PageHomeComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.plantsService.getPlants().subscribe((data) => {
      ...
      //ICI APPELER
      this.categoriesToSend = this.getCategoriesFromPlants(
        this.plantsToDisplay
      ); //stocker retour appel fonction, le tableau de plante
  ...
  methodeGenerale() {
    this.plantsToDisplay = this.allPlants.filter((x) =>
      this.sauvegardeFiltre.includes(x.categorie)
    );
    this.plantsToDisplay = this.plantsToDisplay.filter((x) =>
      x.nom.toLowerCase().includes(this.sauvegardeTexteSaisie.toLowerCase())
    );
  }
}
